    ...
    <div class="p-6">
        <div class="max-w-2xl mx-auto">
            <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
                <h3 class="text-lg font-semibold text-gray-800 mb-4">Edit Feedback</h3>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <select id="lecturer_id" name="lecturer_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                @foreach($lecturers as $lecturer)
                                    <option value="{{ $lecturer->id }}" {{ old('lecturer_id', $feedback->lecturer_id) == $lecturer->id ? 'selected' : '' }}>
                                        {{ $lecturer->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="course_id" name="course_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach($courses as $course)
                                    <option value="{{ $course->id }}" {{ old('course_id', $feedback->course_id) == $course->id ? 'selected' : '' }}>
                                        {{ $course->title }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="category" name="category" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                <option value="general" {{ old('category', $feedback->category) == 'general' ? 'selected' : '' }}>General</option>
                                <option value="course" {{ old('category', $feedback->category) == 'course' ? 'selected' : '' }}>Course Related</option>
                                <option value="technical" {{ old('category', $feedback->category) == 'technical' ? 'selected' : '' }}>Technical Issue</option>
                                <option value="suggestion" {{ old('category', $feedback->category) == 'suggestion' ? 'selected' : '' }}>Suggestion</option>
                                <option value="complaint" {{ old('category', $feedback->category) == 'complaint' ? 'selected' : '' }}>Complaint</option>
                                <option value="other" {{ old('category', $feedback->category) == 'other' ? 'selected' : '' }}>Other</option>
                            </select>
                        ...
                        <div>
                            ...
                            <select id="priority" name="priority" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                ...
                                <option value="urgent" {{ old('priority', $feedback->priority) == 'urgent' ? 'selected' : '' }}>Urgent</option>
                            </select>
                        ...
                        <div>
                            ...
                            <textarea id="content" name="content" rows="6" 
                                      ...
                                      placeholder="Please provide detailed feedback..." required>{{ old('content', $feedback->content) }}</textarea>
